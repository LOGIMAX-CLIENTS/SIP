...
/// Refund status of every Rs.1 verification payment — same rows as
/// PennyVerifyHistoryScreen (GET account/verify-bank/penny/history),
/// rendered with refund-specific fields instead of payment fields.
class RefundHistoryScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // Same endpoint as the payment history screen — refund fields are on
    // the same row, so no separate fetch is needed.
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          GradientHeader(title: 'Refund Verification History'),
          Expanded(
              ...
              child: historyAsync.when(
                ...
                error: (err, _) => ListView(
                  ...
                  children: [
                    ...
                    Center(
                      child: Text('Could not load refund history.',
  ...
  Widget _buildCard(PennyVerifyHistoryItem item, bool isDark) {
    final color = _statusColor(item.refundStatus);
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Container(
                ...
                child: Icon(Icons.replay_circle_filled_rounded, color: color, size: 20.sp),
              ),
              SizedBox(width: 12.w),
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    NumericStyledText(
                      item.refundedOn != null
                          ? 'Refunded on ${_formatDate(item.refundedOn)}'
                          : 'Paid on ${_formatDate(item.createdOn)}',
                      fontSize: 11.sp,
                      color: isDark ? Colors.white54 : Colors.black54,
                    ),
                  ],
          ...
          if (item.refundId != null) ...[
            ...
            SizedBox(height: 8.h),
            NumericStyledText(
              'Refund ID: ${item.refundId} • ₹${item.amount}',
              fontSize: 11.sp,
