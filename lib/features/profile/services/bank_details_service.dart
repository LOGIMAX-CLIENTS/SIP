...
class BankDetailsService {
  ...
  /// POST profile/bank-accounts/upi/remove — soft-remove a UPI linked to a
  /// bank account (backend keeps the RPD-proven link for audit).
  Future<void> removeUpi(String cupiId) async {
    final response = await _apiClient.post(
      'profile/bank-accounts/upi/remove',
      data: {'id': cupiId},
    );
    if (response.data == null || response.data['success'] != true) {
      throw Exception(response.data?['message'] ?? 'Could not remove this UPI.');
    }
  }
